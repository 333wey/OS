...
struct sched_class stride_sched_class = {
    ...
    .init = stride_init,// 初始化运行队列
    .enqueue = stride_enqueue,// 将进程加入运行队列
    .dequeue = stride_dequeue,// 将进程从运行队列中移除
    .pick_next = stride_pick_next,// 选择下一个运行的进程
    .proc_tick = stride_proc_tick,// 处理进程的时钟中断
};
